import React, { useEffect, useState } from 'react';
import { Marker } from '@react-google-maps/api';

/**
 * DriverMarker - Animated car marker with rotation based on heading
 */
const DriverMarker = ({ position, heading, driverName, onClick }) => {
  const [currentPosition, setCurrentPosition] = useState(position);
  const [icon, setIcon] = useState(null);

  // Smooth position transition
  useEffect(() => {
    if (!position) return;

    // Animate position change
    const animationDuration = 1000; // 1 second
    const steps = 20;
    const stepDelay = animationDuration / steps;

    let step = 0;
    const startLat = currentPosition.lat;
    const startLng = currentPosition.lng;
    const latDiff = position.lat - startLat;
    const lngDiff = position.lng - startLng;

    const interval = setInterval(() => {
      step++;
      const progress = step / steps;
      
      setCurrentPosition({
        lat: startLat + (latDiff * progress),
        lng: startLng + (lngDiff * progress)
      });

      if (step >= steps) {
        clearInterval(interval);
      }
    }, stepDelay);

    return () => clearInterval(interval);
  }, [position?.lat, position?.lng]);

  // Create custom car icon with rotation
  useEffect(() => {
    if (typeof window === 'undefined' || !window.google) return;

    const carIcon = {
      path: 'M17.402,0H5.643C2.526,0,0,3.467,0,6.584v34.804c0,3.116,2.526,5.644,5.643,5.644h11.759c3.116,0,5.644-2.527,5.644-5.644 V6.584C23.044,3.467,20.518,0,17.402,0z M22.057,14.188v11.665l-2.729,0.351v-4.806L22.057,14.188z M20.625,10.773 c-1.016,3.9-2.219,8.51-2.219,8.51H4.638l-2.222-8.51C2.417,10.773,11.3,7.755,20.625,10.773z M3.748,21.713v4.492l-2.73-0.349 V14.502L3.748,21.713z M1.018,37.938V27.579l2.73,0.343v8.196L1.018,37.938z M2.575,40.882l2.218-3.336h13.771l2.219,3.336H2.575z M19.328,35.805v-7.872l2.729-0.355v10.048L19.328,35.805z',
      fillColor: '#4285F4',
      fillOpacity: 1,
      strokeColor: '#FFFFFF',
      strokeWeight: 2,
      scale: 0.6,
      anchor: new window.google.maps.Point(11.5, 23.5), // Center of the icon
      rotation: heading || 0
    };

    setIcon(carIcon);
  }, [heading]);

  if (!currentPosition || !currentPosition.lat || !currentPosition.lng) {
    return null;
  }

  return (
    <Marker
      position={currentPosition}
      icon={icon}
      title={driverName || 'Driver'}
      onClick={onClick}
      animation={window.google?.maps?.Animation?.DROP}
      zIndex={1000}
    />
  );
};

export default DriverMarker;
